...
For the coordinated faster-Z and connection-priority feature, use the combined
companion produced by `scripts/package_z_setup_speed.py`, pinned to the recorded
568b1cc9 workpiece-focus installation. Follow its generated INSTALL.md for dry-run
and idle service-stop/apply/start commands. It preserves settings and calibration;
the separate matching speed firmware must also be installed for faster Z travel.
The earlier `scripts/package_pi_control_priority.py` remains a priority-only
three-module source update and does not supply the faster-Z backend.
