...
@pytest.fixture(params=["synthetic", "integrated", "thickness", "cooling", "rejection", "honeycomb", "home_cooling"])
def kit(tmp_path, monkeypatch, request):
    ...
    for entry in manifest["files"]:
        ...
        if request.param in {"integrated", "thickness", "cooling", "rejection", "honeycomb", "home_cooling"}:
            try:
                revision = {"integrated": INTEGRATED_REVISION, "thickness": THICKNESS_REVISION,
                            "cooling": COOLING_REVISION, "rejection": REJECTION_REVISION,
                            "honeycomb": HONEYCOMB_REVISION, "home_cooling": HOME_COOLING_REVISION}[request.param]
