...
from scripts.package_thickness_focus import COOLING_REVISION, INTEGRATED_REVISION, THICKNESS_REVISION, package, source


@pytest.fixture(params=["synthetic", "integrated", "thickness", "cooling"])
def kit(tmp_path, monkeypatch, request):
    ...
    for entry in manifest["files"]:
        ...
        if request.param in {"integrated", "thickness", "cooling"}:
            try:
                revision = {"integrated": INTEGRATED_REVISION, "thickness": THICKNESS_REVISION,
                            "cooling": COOLING_REVISION}[request.param]
