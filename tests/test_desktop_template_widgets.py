...
def test_workspace_native_contrast_preview_uses_fitted_cubic_geometry(
    qt_application: QtWidgets.QApplication,
) -> None:
    view = WorkspaceView(Bounds(0.0, 0.0, 220.0, 220.0))
    ...
    view.set_trace_preview([detection], {"contrast-vector"})
    paths = [
        item
        for item in view._trace_items
        if isinstance(item, QtWidgets.QGraphicsPathItem)
    ...
    assert len(paths) == 1
    assert paths[0].path().boundingRect().left() == pytest.approx(40.0)
    assert paths[0].path().boundingRect().right() == pytest.approx(60.0)
    assert paths[0].path().elementCount() > 4
    assert paths[0].pen().color().name().upper() == "#4FE36F"
    legend = [entry[0] for entry in view._overlay_entries["trace"]]
    assert "Selected trace (green)" in legend
