...
def test_desktop_import_failure_changes_nothing(
    qt_application: QtWidgets.QApplication,
    monkeypatch: pytest.MonkeyPatch,
    tmp_path,
) -> None:
    ...
    harness = _harness()
    original_layers = [OperationLayer.from_dict(layer.to_dict()) for layer in harness.document.layers]
    ...
    assert "vector-only" in harness.errors[0]
