...
- Made the Objects operation-layer color swatch a visible 24 px button beside
  the layer name. It reuses the Cuts / Layers color chooser and existing
  undoable layer-update path, refreshes all shared-layer objects and existing
  color surfaces, and preserves every non-color operation setting, including a
  traced vector layer's 0% power and output-disabled state. The File menu now
  groups the existing SVG, G-code, LightBurn, and raster-image actions under one
  **Import** submenu without replacing their action instances, callbacks,
  icons, enabled state, status text, shortcuts, or File-toolbar use.
