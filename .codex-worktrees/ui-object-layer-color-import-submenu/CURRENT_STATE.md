...
## Active object-layer color and File-import UI follow-up

Each Objects row now presents its operation layer as a visible 24 px color
button beside the layer name. The button opens the same current-color-initialized
chooser as Cuts / Layers and emits through the existing `UpdateLayerCommand`
path. A change therefore remains one undoable layer edit, updates every object
sharing that layer plus the workspace, Cuts panel, and bottom palette, and
preserves speed, power, mode, passes, visibility, output state, scan settings,
and power-correction settings. Canceling the chooser makes no project or history
change; an imported-raster trace layer remains at 0% power with output disabled.

The File menu now contains one **Import** submenu after the save/template
commands. It holds the existing SVG, G-code, LightBurn, and raster-image
`QAction` instances in their prior order, so their callbacks, icons, state, and
`Ctrl+I` / `Ctrl+Shift+I` shortcuts remain unchanged and the File toolbar is not
altered.

Focused Windows/offscreen verification passed **115 tests** across Objects/Cuts,
layer-edit queuing, command history, raster-vector trace-layer handling,
workspace raster/native rendering, and the File menu. The complete Windows
suite passed **2,570 tests** with **14 expected platform skips**. Repository-wide
Ruff, `compileall -q laser_aligner`, and `git diff --check` passed. No interactive
GUI, camera, controller, motion, arming, or laser-output test is claimed for this
