...
def test_objects_layer_color_button_uses_shared_undoable_layer_edit_path(
    qt_application: QtWidgets.QApplication,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    layer = OperationLayer(
    ...
    first = SceneObject.rectangle(layer.id, name="Trace one")
    second = SceneObject.rectangle(layer.id, name="Trace two")
    ...
    first_button = _object_layer_color_button(object_panel, 0)
    first_cell = object_panel.tree.itemWidget(
        object_panel.tree.topLevelItem(0), 1
    )
    assert first_cell is not None
    layer_label = first_cell.findChild(QtWidgets.QLabel, "objectLayerName")
    assert layer_label is not None
    assert layer_label.text() == layer.name
    assert first_button.minimumWidth() >= 22
    assert first_button.minimumHeight() >= 22
    assert object_panel.tree.topLevelItem(0).sizeHint(1).height() <= 30
    assert first_button.isEnabled()
    assert first_button.property("layerColor") == layer.color
    assert layer.color in first_button.styleSheet()
    assert "Change operation layer color" in first_button.toolTip()
    assert "Change operation layer color" in first_button.accessibleName()
    ...
    assert changed.output_enabled is False
    assert changed.power_percent == 0.0

    object_buttons = [
        _object_layer_color_button(object_panel, row)
        for row in range(object_panel.tree.topLevelItemCount())
    ]
    assert [button.property("layerColor") for button in object_buttons] == [
        "#2A7BCB",
        "#2A7BCB",
    ]
    assert layer_panel.color_button.property("layerColor") == "#2A7BCB"
    assert "#2a7bcb" in palette._buttons[layer.id].styleSheet().lower()
    assert workspace._items_by_id[first.id].pen().color().name() == "#2a7bcb"
    assert workspace._items_by_id[second.id].pen().color().name() == "#2a7bcb"

    assert history.undo()
    assert document.get_layer(layer.id).color == "#6FAF58"
    assert layer_panel.color_button.property("layerColor") == "#6FAF58"
    assert all(
        _object_layer_color_button(object_panel, row).property("layerColor")
        == "#6FAF58"
        for row in range(object_panel.tree.topLevelItemCount())
    )
