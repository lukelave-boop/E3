...
- Improved imported-raster line/cubic fitting without changing the schema-3
  native path, Qt rendering, topology validation, or planning boundaries.
  Hard-corner candidates must now show a concentrated, direction-consistent turn
  at two contour scales, suppressing pixel-stair false corners while preserving
  intentional rectilinear corners. Smooth spans use tangent-constrained cubics
  with a shared tangent across recursive splits and non-corner anchors;
  predominantly straight corner-bounded spans remain lines, tiny contours remain
  line-only, and fitted controls are bounded by the reviewed image frame.
