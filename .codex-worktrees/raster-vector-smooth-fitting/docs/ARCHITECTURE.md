...
before fitting. A hard-corner candidate must have a consistent turn direction
and concentrated angle at both a short and wider contour scale; isolated raster
stair steps and smoothly distributed curvature therefore do not become locked
corners. Intentional corners remain fixed through optional smoothing.
Corner-bounded, predominantly straight spans use bounded line simplification.
Other spans use endpoint-tangent-constrained cubic Béziers, share one tangent at
recursive splits and non-corner fitting anchors, and may change tangent only at a
classified hard corner. Controls are bounded by both the span envelope and the
image-local frame. Tiny contours remain line-only rather than receiving unstable
micro-curves. Those fitted segments become the native persisted subpath. A
separate adaptive flattening using the
user-visible fitting tolerance supplies only overlay, topology, and complexity
estimates. Straight runs therefore remain native lines while curved regions
remain cubic segments. The result reports raw contour points, fitted segments,
preview-flattened points, and maximum estimated deviation. That deviation is
relative to threshold-derived and optionally smoothed contours; it is not a
...
bounded circular exclusion window at the wider classification scale rather than
pairwise corner comparisons.
