...
- A dedicated **Trace image to vectors…** action for exactly one selected raster
  ...
  one native multi-contour line/cubic PATH while preserving the image frame,
  position, rotation, and mirrors. Pixel-scale false corners are suppressed,
  smooth fitted spans share tangent-continuous joins, and intentional straight
  edges/corners remain line geometry. Replace, Keep, optional source hiding,
  safe-layer creation, and vector creation are one undoable operation
...
authoritative geometry copy. Corner classification must persist across two
contour scales, while adjacent cubics within a non-corner span share a tangent;
the explicit Smoothing value remains a separate optional displacement control.
When planning is requested, E3 applies the complete object transform and then
adaptively flattens the native curve at the fixed 0.025 mm planning tolerance
before the existing G0/G1 pipeline. A trace uses an existing layer
...
- Raster vectorization is intentionally a single-foreground tracer for logos,
  ...
  retain provable separation through their flattening-error envelopes.
