...
are now fit, validate/convert to one authoritative native subpath, reject exact
cubic extrema outside the image-local frame, and derive bounded preview points
from that native geometry without clipping. Bounded exact cubic self/adjacent-arc
checks and adaptive physical-space clearance prove the authoritative contour
forest before the existing 4× rasterized hierarchy comparison. Parent/depth/hole
state and quality metrics remain attached to each native subpath. One compound
native path preserves all selected outers and holes in the original image-local
frame, with the source Transform, source SHA-256, one-command replace/keep/hide
behavior, and output-disabled 0%-power safe-layer behavior unchanged. Dialog
counts identify fitted segments and preview-flattened points rather than claiming
those points are persisted E3 geometry.

The imported-raster fitter now suppresses false hard corners with a bounded
two-scale turn-persistence test: an accepted corner must retain turn direction
and concentrated angle across short and wider contour windows. Predominantly
straight spans between accepted corners remain bounded line geometry. Other
spans use endpoint-tangent-constrained cubics, with one shared tangent across
recursive splits and artificial non-corner anchors; tangent discontinuities are
therefore reserved for classified hard corners. Tiny contours remain line-only,
and tangent controls are intersected with the reviewed image-local frame before
the existing exact frame and topology validators run. The schema-3 native path,
workspace renderer, transient preview flattening, topology proof, persistence,
and planning-flattening boundaries are unchanged.

Focused Windows verification for this fitter increment passes **68 tests** across
the Qt-free vectorizer, offscreen vectorization dialog, and desktop command
integration. Coverage includes false-staircase rejection, intentional rectangle
corner preservation, zero-explicit-smoothing cubic retention, tangent continuity
at every join of a smooth closed contour, image-edge bounds, nested-hole
topology, complexity limits, source identity, and replace/keep undo behavior.
The operator-supplied Coleman stencil was also exercised as an uncommitted local
diagnostic at 80.0 × 30.358974 mm, threshold 122, zero smoothing, and 0.10 mm
fitting tolerance: it produced 50 contours, 580 fitted segments (322 cubics and
258 lines), 1,035 preview points, and 0.099785 mm maximum estimated deviation.
This is automated and local diagnostic evidence only; no physical tracing-quality
or dimensional-accuracy verification is claimed.
...
automated Qt-free and offscreen-widget geometry coverage only. The latest
focused native-geometry, raster-vectorization/dialog, persistence/history,
toolpath safety/fill, planning/cache/digest, preflight, stock-layout, import, and
template run passes **544 tests**. The complete Windows Python 3.14 run,
including schema/migration, workspace, save/reopen, recovery, and all other
repository coverage, passes **2,568 tests with 14 expected platform/capability
skips**. Repository-wide Ruff, `python -m compileall -q laser_aligner`, and the
effective parent-to-working-tree `git diff --check` pass. The immutable reviewed
commit range still records the pre-existing extra EOF blank line corrected in
this uncommitted working tree. No controller, camera, motion, homing, arming,
laser-output, physical tracing-quality, or physical accuracy verification is
