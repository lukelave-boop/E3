...
now use multiscale hard-corner detection, a separately reported trace-quantization
reconstruction bounded by one source-pixel pitch plus its explicit target-sampling
envelope, optional user smoothing, and a default 0.01 mm
Schneider-style final line/cubic fit with Newton reparameterization, shared smooth
split tangents, verified merging, and bidirectional continuous subdivision bounds.
Sample-only acceptance is not used: between-sample bulges and lobes reject.
Generated controls stay inside the source frame, then exact cubic extrema are
checked independently. Bounded exact cubic self/adjacent-arc
...
history state. Existing raster caps remain 67,108,864 pixels in the 4× workspace,
 ...
 preview-flattened points, plus 5,000,000 bounded continuous fit-validation
 subdivision steps. Limit failures recommend simplifying or cleaning the
source artwork.
...
automated Qt-free and offscreen-widget geometry coverage only. The latest focused
high-quality fitter and raster-vectorization run passes **65 tests**, including
synthetic long curves, between-sample lobes, G1 joins, corners, mixed line/curve
geometry, hierarchy, frame bounds, determinism, and bounded workloads. The
complete Windows Python 3.14 run, including schema/migration, workspace,
save/reopen, recovery, and all other repository coverage, passes **2,579 tests
with 14 expected platform/capability skips**. Repository-wide Ruff,
`python -m compileall -q laser_aligner`, and the
