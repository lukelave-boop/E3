...
- A dedicated **Trace image to vectors…** action for exactly one selected raster
  ...
  position, rotation, and mirrors. Replace, Keep, optional source hiding,
...
authoritative geometry copy. Pixel-grid cleanup is a separate, reported
reconstruction whose bound includes one source-pixel pitch plus its explicit
target-sampling envelope; the default final native fit is independently bounded
to 0.01 mm after optional user smoothing. When planning
is requested, E3 applies the complete
object transform and then adaptively flattens the native curve at the fixed
0.025 mm planning tolerance before the existing G0/G1 pipeline. A trace uses an existing layer
