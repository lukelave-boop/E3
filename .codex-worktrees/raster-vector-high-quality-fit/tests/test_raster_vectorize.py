...
def test_fitter_prevents_anisotropic_frame_control_excursion(
    tmp_path: Path,
) -> None:
    pixels = np.full((64, 64, 3), 255, dtype=np.uint8)
    cv2.ellipse(
    ...
    for contour in result.contours:
        ...
        raster_vectorize_module._validate_native_subpath_in_frame(
            contour.native_subpath,
            96.0,
            24.0,
        )
...
def test_edge_touching_preview_is_authoritative_native_flattening(
    ...
    expected = np.asarray(
        flatten_native_path(
            ...
            raster_vectorize_module._preview_flattening_tolerance_mm(
...
def test_curved_frame_positions_use_one_authoritative_geometry_sequence(
    ...
    _assert_authoritative_preview_and_planning(
        ...
        height_mm=24.0,
        preview_tolerance_mm=(
            raster_vectorize_module._preview_flattening_tolerance_mm(tolerance_mm)
        ),
    )


def test_constrained_fit_preserves_hole_topology_without_excursion(
    tmp_path: Path,
) -> None:
    pixels = np.full((64, 64, 3), 255, dtype=np.uint8)
    outer = np.asarray(
...
def test_bounded_component_complexity_recommends_actionable_cleanup(
    ...
    assert "increase fitting tolerance" in message
...
def test_each_geometry_complexity_budget_rejects_with_cleanup_guidance(
    ...
    assert "increase fitting tolerance" in text
...
def test_stair_step_noise_is_not_promoted_to_hard_corners() -> None:
    count = 20_000
