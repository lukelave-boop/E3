...
_CORNER_INNER_MINIMUM_TURN_DEGREES = 20.0
_CORNER_OUTER_MINIMUM_TURN_DEGREES = 30.0
_CORNER_MINIMUM_SCALE_AGREEMENT = 0.55
_MAX_CORNER_WINDOW_POINTS = 24
_MAX_TRACE_CLEANUP_PIXEL_PITCH = 1.0
_TRACE_CLEANUP_TARGET_SAMPLING_FRACTION = 0.2
_MAX_SMOOTHING_RADIUS_POINTS = 64
_MAX_TANGENT_WINDOW_POINTS = 12
...
_COMPLEXITY_ADVICE = (
    "Increase the minimum feature size, increase fitting tolerance, adjust the "
...
class RasterVectorizationOptions:
    ...
    simplification_tolerance_mm: float = 0.01
...
class RasterVectorizedContour:
    ...
    trace_cleanup_deviation_mm: float = 0.0
    ...
    def __post_init__(self) -> None:
        ...
        trace_cleanup_deviation = _finite(
            self.trace_cleanup_deviation_mm,
            "contour trace_cleanup_deviation_mm",
        )
        deviation = _finite(
        ...
        if trace_cleanup_deviation < 0.0:
            raise ValueError("contour trace cleanup deviation cannot be negative")
        if deviation < max(
            fitting_error,
            smoothing_displacement,
            trace_cleanup_deviation,
        ):
            raise ValueError(
...
class RasterVectorizationResult:
    ...
    def metadata(self) -> dict[str, object]:
        ...
        return {
            ...
            "raster_vectorization_mean_fitting_error_mm": (
                mean_fitting_error
            ),
            ...
            "raster_vectorization_max_trace_cleanup_deviation_mm": max(
                contour.trace_cleanup_deviation_mm for contour in self.contours
            ),
...
class _FittedContour:
    ...
    trace_cleanup_deviation_mm: float
    smoothing_displacement_mm: float
    max_fitting_error_mm: float
    mean_fitting_error_mm: float
    rms_fitting_error_mm: float
    fitting_error_sample_count: int
    hard_corner_count: int
    recursive_split_count: int
    merged_segment_count: int
    longest_smooth_span_segment_count: int
...
class _FittedPiece:
    ...
    hard_end: bool = False
    sample_error_sum_mm: float = 0.0
    sample_squared_error_sum_mm2: float = 0.0
    sample_count: int = 0
...
class _ComplexityBudget:
    ...
    def ensure_fitted_segments(self, count: int) -> None:
        if self.fitted_segments + count > MAX_RASTER_VECTORIZATION_FITTED_SEGMENTS:
            _raise_complexity(
                "Raster vectorization requires more than "
                f"{MAX_RASTER_VECTORIZATION_FITTED_SEGMENTS:,} fitted segments"
...
def _point_to_lines_distance(
    ...
    cross = np.abs(
        delta[:, 0] * (starts[:, 1] - samples[:, 1])
        - (starts[:, 0] - samples[:, 0]) * delta[:, 1]
    )
    return np.divide(
        cross,
        lengths,
...
def _corner_scale_metrics(
    ...
    offsets = sorted({max(1, span // 3), max(1, 2 * span // 3)})
    for offset in offsets:
        residual = np.maximum(
            residual,
            _point_to_lines_distance(
                np.roll(points, offset, axis=0),
                previous,
                points,
            ),
        )
...
def _local_turn_metrics(points: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    previous_steps = points - np.roll(points, 1, axis=0)
    following_steps = np.roll(points, -1, axis=0) - points
    denominator = np.maximum(
        np.linalg.norm(previous_steps, axis=1)
        * np.linalg.norm(following_steps, axis=1),
        1e-15,
    )
    turns = np.arccos(
        np.clip(
            np.sum(previous_steps * following_steps, axis=1) / denominator,
            -1.0,
            1.0,
        )
    )
    orientation = (
        previous_steps[:, 0] * following_steps[:, 1]
        - previous_steps[:, 1] * following_steps[:, 0]
    )
    return turns, orientation


def _corner_indices(points: np.ndarray, tolerance_mm: float = 0.01) -> list[int]:
    count = len(points)
    if count < 8:
    ...
    inner_span = max(
        3,
        min(
            _MAX_CORNER_WINDOW_POINTS // 2,
            int(math.ceil(max(4.0 * step_mm, 4.0 * tolerance_mm) / step_mm)),
        ),
    )
    outer_span = max(
        inner_span + 2,
        min(
            ...
            int(math.ceil(max(10.0 * step_mm, 8.0 * tolerance_mm) / step_mm)),
        ),
    )
    local_turns, local_orientation = _local_turn_metrics(points)
    if count < 4 * outer_span:
        return [
            int(index)
            for index in np.flatnonzero(local_turns >= math.radians(30.0))
        ]
    ...
    # A rasterized polygon corner is commonly represented by a one-pixel chamfer:
    # two adjacent, genuine direction changes rather than one sample at the ideal
    # mathematical intersection.  The broad detector above establishes that the
    # region has straight arms and a persistent turn; only then refine that region
    # to its local direction changes.  This preserves both edges of the chamfer as
    # lines without promoting the repeated turns of an otherwise straight pixel
    # staircase to hard corners.
    refined: set[int] = set()
    local_offsets = np.arange(-inner_span, inner_span + 1, dtype=np.int64)
    for index in selected:
        neighbourhood = (index + local_offsets) % count
        eligible = [
            int(candidate)
            for candidate in neighbourhood
            if local_turns[candidate] >= math.radians(30.0)
            and local_orientation[candidate] * outer_orientation[index] > 0.0
        ]
        if not eligible:
            refined.add(index)
            continue
        ...
        while changed:
            ...
            for candidate in eligible:
                ...
                if any(
                    ...
                    changed = True
        refined.update(cluster)
    return sorted(refined)


def _straight_trace_runs(
    points: np.ndarray,
    tolerance_mm: float,
) -> list[np.ndarray]:
    """Find long raster runs whose broad geometry is genuinely linear."""

    count = len(points)
    span = min(_MAX_CORNER_WINDOW_POINTS, max(4, count // 32))
    if count < 4 * span:
        return []
    turns, _orientation, residual = _corner_scale_metrics(points, span)
    steps = np.linalg.norm(np.roll(points, -1, axis=0) - points, axis=1)
    positive_steps = steps[steps > 1e-12]
    if not len(positive_steps):
        return []
    step_mm = float(np.median(positive_steps))
    local_allowance = max(tolerance_mm, 0.8 * step_mm)
    locally_straight = (
        (turns <= math.radians(5.0)) & (residual <= local_allowance)
    )
    starts = np.flatnonzero(locally_straight & ~np.roll(locally_straight, 1))
    if not len(starts):
        return []
    minimum_length = max(2.0, 50.0 * tolerance_mm, 32.0 * step_mm)
    runs: list[np.ndarray] = []
    for raw_start in starts:
        indices: list[int] = []
        index = int(raw_start)
        while locally_straight[index] and len(indices) < count:
            indices.append(index)
            index = (index + 1) % count
        if len(indices) < 2 * span:
            continue
        run = np.asarray(indices, dtype=np.int64)
        start = points[run[0]]
        end = points[run[-1]]
        chord_length = float(np.linalg.norm(end - start))
        if chord_length < minimum_length:
            continue
        starts_array = np.repeat(start[None, :], len(run), axis=0)
        ends_array = np.repeat(end[None, :], len(run), axis=0)
        if float(
            np.max(_point_to_lines_distance(points[run], starts_array, ends_array))
        ) <= local_allowance:
            runs.append(run)
    return runs
...
def _flatten_cleanup_segment(
    segment: _FittedSegment,
    tolerance_mm: float,
) -> list[np.ndarray]:
    if isinstance(segment, _LineSegment):
        return [segment.start.copy(), segment.end.copy()]
    controls = np.vstack(
        (segment.start, segment.control_1, segment.control_2, segment.end)
    )
    output = [segment.start.copy()]
    stack: list[tuple[np.ndarray, int]] = [(controls, 0)]
    while stack:
        current, depth = stack.pop()
        starts = np.repeat(current[0][None, :], 2, axis=0)
        ends = np.repeat(current[3][None, :], 2, axis=0)
        flatness = float(
            np.max(_point_to_lines_distance(current[1:3], starts, ends))
        )
        if flatness <= tolerance_mm or depth >= _MAX_FLATTEN_RECURSION:
            output.append(current[3].copy())
            continue
        left, right = _split_cubic_controls(current)
        stack.append((right, depth + 1))
        stack.append((left, depth + 1))
    return output


def _cleanup_trace_quantization(
    points: np.ndarray,
    corners: list[int],
    straight_runs: list[np.ndarray],
    frame_minimum: np.ndarray,
    frame_maximum: np.ndarray,
    fitting_tolerance_mm: float,
    budget: _ComplexityBudget,
    *,
    maximum_pixel_pitch: float = _MAX_TRACE_CLEANUP_PIXEL_PITCH,
) -> tuple[np.ndarray, list[int], list[int], float]:
    """Reconstruct a smooth target within one reported source-pixel uncertainty.

    This stage defines the accepted reconstruction target before user smoothing and
    before the independently bounded Bézier fit.  Its displacement is reported
    separately, so the fitting tolerance is never repurposed as implicit smoothing.
    """

    if len(points) < 3:
        return points.copy(), list(corners), [], 0.0
    raw_soft_anchors = sorted(
        {
            int(endpoint)
            for run in straight_runs
            for endpoint in (run[0], run[-1])
        }
    )
    if maximum_pixel_pitch <= 0.0:
        return points.copy(), list(corners), raw_soft_anchors, 0.0
    steps = np.linalg.norm(np.roll(points, -1, axis=0) - points, axis=1)
    positive_steps = steps[steps > 1e-12]
    if not len(positive_steps):
        return points.copy(), list(corners), raw_soft_anchors, 0.0
    sampling_tolerance = max(
        1e-5,
        fitting_tolerance_mm * _TRACE_CLEANUP_TARGET_SAMPLING_FRACTION,
    )
    pixel_uncertainty = (
        maximum_pixel_pitch
        * RASTER_VECTORIZATION_OVERSAMPLE_FACTOR
    ...
    if pixel_uncertainty <= sampling_tolerance:
        return points.copy(), list(corners), raw_soft_anchors, 0.0
    cleanup_tolerance = max(fitting_tolerance_mm, pixel_uncertainty)
    corner_set = set(corners)
    straight_pairs = {
        (int(run[0]), int(run[-1])) for run in straight_runs
    }
    anchors = _fitting_anchors(
        len(points),
        [*corners, *raw_soft_anchors],
    )
    pieces: list[_FittedPiece] = []
    for start, end, span in _contour_spans(points, anchors):
        hard_start = start in corner_set
        hard_end = end in corner_set
        start_tangent = (
            _endpoint_tangent(span, cleanup_tolerance, at_start=True)
            if hard_start
            else _closed_forward_tangent(points, start, cleanup_tolerance)
        )
        end_tangent = (
            _endpoint_tangent(span, cleanup_tolerance, at_start=False)
            if hard_end
            else -_closed_forward_tangent(points, end, cleanup_tolerance)
        )
        pieces.extend(
            _fit_span(
                span,
                ...
                frame_maximum,
                budget,
                allow_line=(start, end) in straight_pairs,
                hard_start=hard_start,
                hard_end=hard_end,
                permit_intrinsic_lines=False,
            )
        )
    pieces = _merge_smooth_pieces(
        ...
        frame_maximum,
        budget,
        minimum_segment_count=max(4, len(corners)),
    )
    target: list[np.ndarray] = [pieces[0].segment.start.copy()]
    target_corners: set[int] = set()
    target_soft_anchors: set[int] = set()
    for piece in pieces:
        start_index = len(target) - 1
        target_soft_anchors.add(start_index)
        if piece.hard_start:
            target_corners.add(start_index)
        flattened = _flatten_cleanup_segment(piece.segment, sampling_tolerance)
        target.extend(flattened[1:])
        end_index = len(target) - 1
        target_soft_anchors.add(end_index)
        if piece.hard_end:
            target_corners.add(end_index)
    if len(target) > 1 and float(np.linalg.norm(target[-1] - target[0])) <= 1e-12:
        closing_index = len(target) - 1
        target.pop()
        if closing_index in target_corners:
            target_corners.add(0)
        if closing_index in target_soft_anchors:
            target_soft_anchors.add(0)
        target_corners.discard(closing_index)
        target_soft_anchors.discard(closing_index)
    cleanup_error = (
        max(piece.segment.fitting_error_mm for piece in pieces) + sampling_tolerance
    )
    return (
        np.asarray(target, dtype=np.float64),
        sorted(target_corners),
        sorted(target_soft_anchors),
        cleanup_error,
    )
...
def _fitting_anchors(point_count: int, corners: list[int]) -> list[int]:
    ...
    target = 8 if not anchors else 2
...
def _tangent_window(points: np.ndarray, tolerance_mm: float) -> int:
    if len(points) <= 2:
        return 1
    ...
    if not len(positive):
        return 1
    step_mm = float(np.median(positive))
    target_length = max(6.0 * step_mm, 8.0 * tolerance_mm)
    return max(
        2,
        min(
            _MAX_TANGENT_WINDOW_POINTS,
            len(points) - 1,
            int(math.ceil(target_length / step_mm)),
        ),
    )


def _pca_tangent(samples: np.ndarray, orientation: np.ndarray) -> np.ndarray:
    if len(samples) <= 2:
        tangent = _unit_direction(orientation)
    else:
        centered = samples - np.mean(samples, axis=0)
        _u, _singular, vectors = np.linalg.svd(centered, full_matrices=False)
        tangent = _unit_direction(vectors[0])
    if tangent is None:
        tangent = _unit_direction(orientation)
    if tangent is None:
...
def _endpoint_tangent(
    ...
    *,
    ...
    window = _tangent_window(points, tolerance_mm)
    if at_start:
        samples = points[: window + 1]
        orientation = samples[-1] - samples[0]
    else:
        samples = points[-window - 1 :]
        orientation = samples[0] - samples[-1]
    return _pca_tangent(samples, orientation)


def _center_forward_tangent(
    points: np.ndarray,
    split: int,
    tolerance_mm: float,
) -> np.ndarray:
    window = min(
        _tangent_window(points, tolerance_mm),
        split,
        len(points) - split - 1,
    )
    samples = points[split - window : split + window + 1]
    return _pca_tangent(samples, samples[-1] - samples[0])


def _closed_forward_tangent(
    points: np.ndarray,
    index: int,
    tolerance_mm: float,
) -> np.ndarray:
    window = _tangent_window(points, tolerance_mm)
    indices = (np.arange(-window, window + 1) + index) % len(points)
...
def _generate_bezier_controls(
    ...
    first = points[0]
    last = points[-1]
    base = (basis_0 + basis_1)[:, None] * first + (
        basis_2 + basis_3
    )[:, None] * last
    residual = points - base
    column_1 = basis_1[:, None] * start_tangent
    column_2 = basis_2[:, None] * end_tangent
    matrix = np.column_stack((column_1.reshape(-1), column_2.reshape(-1)))
    handles, _residuals, rank, _singular = np.linalg.lstsq(
        matrix,
        residual.reshape(-1),
        rcond=None,
    )
    chord = float(np.linalg.norm(last - first))
    epsilon = max(1e-12, chord * 1e-6)
    if (
        rank < 2
        or not np.all(np.isfinite(handles))
        or handles[0] <= epsilon
        or handles[1] <= epsilon
    ):
        handles = np.asarray((chord / 3.0, chord / 3.0), dtype=np.float64)
    return np.vstack(
        (
            first,
            first + handles[0] * start_tangent,
            last + handles[1] * end_tangent,
            last,
        )
    )


def _cubic_derivatives(controls: np.ndarray, parameter: float) -> tuple[np.ndarray, np.ndarray]:
...
def _split_cubic_controls(controls: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    first = (controls[:-1] + controls[1:]) * 0.5
    second = (first[:-1] + first[1:]) * 0.5
    midpoint = (second[0] + second[1]) * 0.5
    return (
...
def _point_to_target_polyline(
    ...
    projected = starts + ratios[:, None] * delta
...
def _validate_curve_fit(
    ...
    sample_count = 0
    curve_stack: list[tuple[np.ndarray, int]] = [(controls, 0)]
    while curve_stack:
        current, depth = curve_stack.pop()
        budget.consume_fit_validation_step()
        midpoint = _cubic_values(
            current[0],
            current[1],
            current[2],
            current[3],
        ...
        distance, split_index = _point_to_target_polyline(midpoint, target)
        radius = float(np.max(np.linalg.norm(current - midpoint, axis=1)))
        upper = distance + radius
        if upper <= tolerance_mm:
            maximum = max(maximum, upper)
            sample_sum += distance
            sample_squared_sum += distance * distance
            sample_count += 1
            continue
        if distance > tolerance_mm or depth >= _MAX_FIT_VALIDATION_RECURSION:
        ...
        left, right = _split_cubic_controls(current)
        curve_stack.append((right, depth + 1))
        curve_stack.append((left, depth + 1))

    for index, (start, end) in enumerate(
        zip(target[:-1], target[1:], strict=True)
    ):
        target_stack: list[tuple[np.ndarray, np.ndarray, float, float, int]] = [
            (start, end, float(parameters[index]), float(parameters[index + 1]), 0)
        ]
        while target_stack:
            first, last, first_u, last_u, depth = target_stack.pop()
            budget.consume_fit_validation_step()
            midpoint = (first + last) * 0.5
            parameter = (first_u + last_u) * 0.5
            curve_point = _cubic_values(
                controls[0],
                controls[1],
                controls[2],
                controls[3],
            ...
            distance = float(np.linalg.norm(midpoint - curve_point))
            upper = distance + 0.5 * float(np.linalg.norm(last - first))
            if upper <= tolerance_mm:
                maximum = max(maximum, upper)
                sample_sum += distance
                sample_squared_sum += distance * distance
            ...
            target_stack.append(
                (midpoint, last, parameter, last_u, depth + 1)
            )
            target_stack.append(
                (first, midpoint, first_u, parameter, depth + 1)
            )
    return _FitValidation(
        True,
        maximum,
...
def _controls_inside_frame(
    controls: np.ndarray,
    frame_minimum: np.ndarray,
    frame_maximum: np.ndarray,
) -> bool:
    return bool(
        np.all(controls >= frame_minimum - _NATIVE_FRAME_EPSILON_MM)
        and np.all(controls <= frame_maximum + _NATIVE_FRAME_EPSILON_MM)
    )
...
def _attempt_cubic_piece(
    ...
    *,
    hard_start: bool = False,
    hard_end: bool = False,
) -> tuple[_FittedPiece | None, int]:
    parameters = _chord_parameters(points)
    split_index = max(1, min(len(points) - 2, len(points) // 2))
    best_error = math.inf
    for _iteration in range(_MAX_REPARAMETERIZATION_ITERATIONS + 1):
        ...
        if (
            ...
            and _controls_inside_frame(controls, frame_minimum, frame_maximum)
        ):
        ...
        if np.array_equal(refined, parameters):
            break
        parameters = refined
    return None, max(1, min(len(points) - 2, split_index))
...
def _span_is_intrinsically_linear(
    points: np.ndarray,
    tolerance_mm: float,
) -> bool:
    """Distinguish a true straight run from a short acceptable curve chord."""

    if len(points) < 5:
        return False
    delta = points[-1] - points[0]
    chord_direction = _unit_direction(delta)
    if chord_direction is None:
        return False
    steps = np.linalg.norm(np.diff(points, axis=0), axis=1)
    positive_steps = steps[steps > 1e-12]
    minimum_length = max(
        10.0 * tolerance_mm,
        4.0 * float(np.median(positive_steps)) if len(positive_steps) else 0.0,
    )
    if float(np.linalg.norm(delta)) < minimum_length:
        return False
    start_direction = _endpoint_tangent(points, tolerance_mm, at_start=True)
    end_direction = -_endpoint_tangent(points, tolerance_mm, at_start=False)
    minimum_alignment = math.cos(math.radians(2.0))
    return bool(
        float(np.dot(start_direction, chord_direction)) >= minimum_alignment
        and float(np.dot(end_direction, chord_direction)) >= minimum_alignment
    )
...
def _fit_span(
    ...
    *,
    ...
    hard_start: bool,
    hard_end: bool,
    permit_intrinsic_lines: bool = True,
    depth: int = 0,
) -> list[_FittedPiece]:
    if allow_line or (
        permit_intrinsic_lines
        and _span_is_intrinsically_linear(points, tolerance_mm)
    ):
    ...
    candidate, split = _attempt_cubic_piece(
        points,
        tolerance_mm,
        start_tangent,
        end_tangent,
    ...
    if depth >= _MAX_FIT_RECURSION:
        ...
        for index in range(len(points) - 1):
            ...
            pieces.append(line)
        budget.ensure_fitted_segments(len(pieces))
        return pieces

    split = max(1, min(len(points) - 2, split))
    forward = _center_forward_tangent(points, split, tolerance_mm)
    ...
    left = _fit_span(
        ...
        hard_start=hard_start,
        hard_end=False,
        permit_intrinsic_lines=permit_intrinsic_lines,
        depth=depth + 1,
    )
    right = _fit_span(
        points[split:],
...
def _merge_smooth_pieces(
    ...
    *,
    ...
    while len(pieces) > minimum_segment_count and index + 1 < len(pieces):
        ...
        if isinstance(first.segment, _LineSegment) and isinstance(
            second.segment,
            _LineSegment,
        ):
        ...
        if merged is None:
            merged, _split = _attempt_cubic_piece(
                target,
                tolerance_mm,
                first.start_tangent,
                second.end_tangent,
...
def _fit_contour(
    ...
    height_mm: float,
    *,
    trace_cleanup_pixel_pitch: float = _MAX_TRACE_CLEANUP_PIXEL_PITCH,
) -> _FittedContour:
    fit_tolerance = options.simplification_tolerance_mm
    frame_minimum = np.asarray((-width_mm / 2.0, -height_mm / 2.0))
    frame_maximum = np.asarray((width_mm / 2.0, height_mm / 2.0))
    raw_corners = _corner_indices(raw_points, fit_tolerance)
    straight_runs = _straight_trace_runs(raw_points, fit_tolerance)
    (
        cleaned,
        corners,
        soft_anchors,
        trace_cleanup_deviation,
    ) = _cleanup_trace_quantization(
        raw_points,
        raw_corners,
        straight_runs,
        frame_minimum,
        ...
        fit_tolerance,
        budget,
        maximum_pixel_pitch=trace_cleanup_pixel_pitch,
    )
    ...
    anchors = _fitting_anchors(len(smoothed), [*corners, *soft_anchors])
    corner_set = set(corners)
    ...
    for start, end, span in _contour_spans(smoothed, anchors):
        ...
        hard_end = end in corner_set
        start_tangent = (
            _endpoint_tangent(span, fit_tolerance, at_start=True)
            if hard_start
            else _closed_forward_tangent(smoothed, start, fit_tolerance)
        )
        end_tangent = (
            _endpoint_tangent(span, fit_tolerance, at_start=False)
            if hard_end
            else -_closed_forward_tangent(smoothed, end, fit_tolerance)
        )
        pieces.extend(
            _fit_span(
                span,
    ...
    return _FittedContour(
        segments=segments,
        trace_cleanup_deviation_mm=trace_cleanup_deviation,
        smoothing_displacement_mm=smoothing_displacement,
        max_fitting_error_mm=max(segment.fitting_error_mm for segment in segments),
...
def _preview_flattening_tolerance_mm(fitting_tolerance_mm: float) -> float:
    """Return the display-only flattening tolerance, separate from fit acceptance."""

    return max(0.001, min(0.025, fitting_tolerance_mm * 0.5))
...
def vectorize_prepared_raster(
    ...
    *,
    ...
    for original_index in selected_indices:
        ...
        preview_tolerance_mm = _preview_flattening_tolerance_mm(
            options.simplification_tolerance_mm
        )
        candidate: RasterVectorizedContour | None = None
        for cleanup_pixel_pitch in (1.0, 0.75, 0.5, 0.25, 0.0):
            fitted = _fit_contour(
                physical,
                options,
                budget,
                width_mm,
                height_mm,
                trace_cleanup_pixel_pitch=cleanup_pixel_pitch,
            )
            ...
            preview_budget = _ComplexityBudget()
            ...
            candidate = RasterVectorizedContour(
            ...
            standalone = (
                candidate
                if candidate.parent_index is None and candidate.depth == 0
                else replace(candidate, parent_index=None, depth=0, is_hole=False)
            )
            try:
                ...
                if (
                    "topology remains ambiguous" not in str(exc)
                    or cleanup_pixel_pitch == 0.0
                ):
                    raise
                continue
            budget.add_fitted_segments(candidate.fitted_segment_count)
            budget.add_preview_points(candidate.preview_flattened_point_count)
            results.append(candidate)
            break
        if candidate is None or len(results) != selected_map[original_index] + 1:
            raise RasterVectorizationError(
                "Raster contour fitting did not produce a bounded native path"
            )
    if not results:
        raise RasterVectorizationError(
