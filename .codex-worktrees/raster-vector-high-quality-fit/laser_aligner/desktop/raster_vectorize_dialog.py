...
class RasterVectorizationDialog(QtWidgets.QDialog):
    ...
    def _build_detection_group(self) -> Any:
        ...
        self.smoothing_spin.setToolTip(
            "Apply intentional user smoothing after the separately reported "
            "pixel-grid trace cleanup, while retaining detected sharp corners."
        )
        ...
        self.simplification_spin.setSingleStep(0.005)
        ...
        self.simplification_spin.setToolTip(
            "Hard maximum deviation accepted for native line and Bézier fitting, "
            "in millimetres at the image's displayed size. Automatic trace cleanup, "
            "user smoothing displacement, and preview flattening are measured "
    ...
    def _preview_succeeded(
        ...
        self.stats_label.setText(
            ...
            "maximum validated fit "
            f"{max(c.max_fitting_error_mm for c in result.contours):.3f} mm  ·  "
            "trace cleanup bound "
            f"{max(c.trace_cleanup_deviation_mm for c in result.contours):.3f} mm  ·  "
            "user smoothing "
            f"{max(c.smoothing_displacement_mm for c in result.contours):.3f} mm"
        )
