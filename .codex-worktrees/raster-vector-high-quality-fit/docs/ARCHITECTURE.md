...
before fitting. Genuine corners require agreeing short- and wide-scale turns,
consistent orientation, and locally straight arms; a bounded refinement retains
the small chamfers by which a raster represents a real polygon corner. Repeated
stair-step turns without a persistent broad turn are not promoted to hard
corners. Long locally linear runs become soft fitting anchors rather than hard
tangent breaks.

Pixel quantization cleanup and final fitting are separate. The cleanup stage runs
the same conservative recursive cubic fitter against the raw 4× contour with a
limit derived from one source-pixel pitch. Intrinsic short curve chords
are disabled in this pass, while confirmed straight runs may remain lines. Its
validated maximum deviation from the raw contour is reported as the **trace
cleanup bound**. Optional user smoothing then operates on that accepted cleanup
target and reports its own maximum point displacement. Reducing cleanup is
retried automatically if a candidate contour cannot pass the unchanged native
topology proof. The cleanup curve is adaptively sampled into the accepted target;
that sampling envelope is included in the reported cleanup bound. At the default
0.01 mm final tolerance the sampling term is 0.002 mm, so the cleanup bound is at
most one source-pixel pitch plus 0.002 mm.

Final reconstruction is a self-contained Schneider/Graphics Gems FitCurves-style
solver: chord-length parameters, constrained least-squares handles, Newton
reparameterization, shared split tangents for G1 smooth joins, recursive splitting
only after a failed bound, and verified adjacent-span merging. Hard corners never
share a forced tangent. Whole confirmed straight spans persist as native lines;
mixed line/cubic pairs are not merged away. Generated controls remain inside the
reviewed image frame.

Fit acceptance is bidirectional and continuous rather than sample-only. For the
cubic-to-target direction, recursive de Casteljau cells use the cell midpoint's
distance to the target polyline plus the maximum control-hull radius about that
midpoint as a conservative upper bound. For target-to-cubic, each recursively
halved target interval uses its midpoint's distance to the corresponding cubic
parameter plus half the target interval length; Euclidean distance is
1-Lipschitz, so this also safely overestimates the interval. A cell is accepted
only when its upper bound is within the requested tolerance; recursion or work-
budget exhaustion rejects the candidate. The reported maximum fitting deviation
is the largest accepted conservative bound. Reported mean and RMS values are
diagnostic midpoint residuals, not substitute acceptance criteria.

Those fitted segments become the native persisted subpath. A separate display
flattening supplies only overlay/topology samples; it is excluded from source-fit
deviation and is not persisted. The maximum estimated source deviation is the
conservative sum of trace-cleanup bound, optional smoothing displacement, and
final fitting bound. None of these values is a physical-accuracy certification of
the source artwork.
...
guidance to increase minimum feature size or fitting tolerance, adjust threshold,
...
- 5,000,000 bounded continuous fit-validation subdivision steps; and
