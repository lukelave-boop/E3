"""Offline image validation and explicit operator-run F103RET6 USB firmware update.
...
VECTOR_TABLE_SIZE = 121 * 4  # ST startup includes reserved slots and BootRAM.
...
PREFIX = "E3AUX1"
UPDATER_IDENTITIES = {
    "E3AUX1 UPDATER 0.1.0 BOARD=0103E013",
    "E3AUX1 UPDATER 0.2.0 BOARD=0103E013",
}
IDENTITIES = {
    "E3AUX1 UPDATER 0.2.0 BOARD=0103E013": "UPDATER",
    "E3AUX1 UPDATER 0.1.0 BOARD=0103E013": "UPDATER",
    "E3AUX1 APP 0.1.0 BOARD=0103E013": "APP",
    "E3AUX1 APP 0.2.0 BOARD=0103E013 MODE=BENCH OUTPUTS=DISABLED": "BENCH",
}
...
class Link:
    ...
    def identity(self) -> str:
        ...
        for _ in range(128):
            ...
            if response == "E3AUX1 UPDATER 0.2.0 BOARD=0103E013":
                return "UPDATER"
            if response.startswith("FIRMWARE_NAME:Marlin "):
                marlin = True
            elif response == "Cap:E3_USB_UPDATER_F103_V1:1":
                capability = True
            elif response == "ok" or response.startswith("ok "):
                if marlin and capability:
                    return "MARLIN_F103"
                raise FirmwareError("Firmware is not the F103 USB-update build")
            elif response.startswith(("ERR", "Error", "E3AUX1")):
                raise FirmwareError(f"Unexpected firmware identity: {response}")
        raise FirmwareError("No confirmed F103 updater or application identity")

    def hold_updater(self) -> None:
        if self.identity() == "MARLIN_F103":
        ...
        if self.identity() != "UPDATER":
            raise FirmwareError("Expected F103 updater before erase")
