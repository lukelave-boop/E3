...
from firmware.ender_aux_f103.host import FLASH_END, VECTOR_BASE, pack_image, validate_image  # noqa: E402
...
def audit_startup(elf_path: Path) -> None:
    ...
    emulator.mem_map(0x40021000, 0x2000)
...
def audit(elf_path: Path, objdump: Path) -> bytes:
    ...
    with elf_path.open("rb") as handle:
        ...
        if vectors is None or vectors["sh_addr"] != VECTOR_BASE or vectors["sh_size"] != 484:
