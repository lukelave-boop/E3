...
GPIO_PORTS = (0x40010800, 0x40010C00, 0x40011000)
...
class PlatformModel:
    ...
    def __init__(self, elf_path: Path):
        ...
        self.uc.mem_map(0x1FFFF000, 0x1000)
        ...
        with elf_path.open("rb") as handle:
            ...
            require(self.symbols.get("__image_start") == 0x08007000, "ELF must be the production updater")
    ...
    def set_target(self, *, device: int = 0x414, size: int = 512, ready: bool = True) -> None:
    ...
    def _write_mmio(self, uc, _access, address, size, value, _data):
        if address == FLASH_KEYR:
            ...
            if value & (1 << 6):
                ...
                first = self.word(0x40022014)
                require(first % 2048 == 0 and APP_START <= first <= FLASH_END - 2048, "Protected or unaligned page erase")
                self.erased.append(first)
                uc.mem_write(first, b"\xff" * 2048)
                self.flash_cr &= ~(1 << 6)
        elif address in (0x40022008, 0x4002201C):
            raise RuntimeError("Option-byte mutation requested")
        for port in GPIO_PORTS:
            if address == port + 0x10:
    ...
    def _program_flash(self, uc, _access, address, size, value, _data):
        ...
        require(size == 2, "F103 must program exactly one halfword")
        previous = int.from_bytes(uc.mem_read(address, size), "little")
        require(previous == 0xFFFF, "Flash programming attempted a 0-to-1 bit transition")
...
def run_tests(model: PlatformModel) -> int:
    cases = 0
    invalid_addresses = (
        0, FLASH_START, 0x0800FFFC, 0x08007000, APP_START - 4, APP_START - 1,
    ...
    for address, value in ((APP_START, 0x12345678), (FLASH_END - 4, 0x76543210)):
        ...
        require([item[0] for item in model.program_writes] == list(range(address, address + 4, 2)),
    ...
    require(model.erased == list(range(APP_START, FLASH_END, 2048)), "Application erase did not touch exactly the 224 application pages")
    ...
    # Exercise the F1 CRL/CRH mode fields and BSRR pin levels.
    for port in GPIO_PORTS:
        model.output_levels[port] = 0xFFFF
        model.uc.mem_write(port, struct.pack("<II", 0xBBBBBBBB, 0xBBBBBBBB))
    model.clear_events()
    ...
    def mode(port, pin):
        return (model.word(port + (4 if pin >= 8 else 0)) >> ((pin % 8) * 4)) & 15
    for port, pins in inactive_pins.items():
        for pin in pins:
            require(not model.output_levels[port] & (1 << pin), "Output was not low")
            require(mode(port, pin) == 2, "Output mode must be 2 MHz push-pull")
    require(model.output_levels[port_c] & 8 and mode(port_c, 3) == 2, "Motor enable was not inactive")
    for pin in (13, 14):
        require(mode(port_c, pin) == 4, "Probe must remain floating input")
    require(model.word(0x40010004) & (7 << 24) == 2 << 24, "JTAG must release motor pins while retaining SWD")
...
def main() -> int:
    ...
    parser.add_argument("--elf", type=Path, default=Path(__file__).resolve().parents[2] / "build/ender_aux_f103/updater.elf")
