"""Run the compiled Cortex-M3 updater core against fake flash in Unicorn.
...
from build import FLAGS, HERE, ROOT


def toolchain_bin(requested):
    from pathlib import Path
    return Path(requested) if requested else ROOT / "build/firmware-tools/bin"
def tool(folder, name):
    return str(folder / ("arm-none-eabi-" + name + ".exe"))
...
""", encoding="utf-8")
    ...
    subprocess.run([
        ...
        "-T", str(linker), str(HERE / "core_harness.c"),
    ...
    if finished[0]:
        raise RuntimeError(f"Cortex-M3 core test failure code/line: {finished[0]}")
    print("Compiled Cortex-M3 core tests passed (fake flash/UART; no physical hardware).")
    return 0
