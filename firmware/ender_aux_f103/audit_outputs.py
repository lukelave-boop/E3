...
def audit_outputs(path: Path) -> None:
    ...
    def record(_uc, _access, address, size, value, _data):
        if 0x40010000 <= address < 0x40013000:
    ...
    # F1 BRR bit 0 resets the output without touching other GPIO outputs.
    if (not completed or uc.mem_read(speeds, 2) != b"\x00\x00"
            or (0x40010814, 4, 1) not in writes
            or (0x40011014, 4, 1) not in writes):
