...
static int test_crc_and_header(void) {
    ...
    CHECK(!image_metadata_valid(E3_BOARD_ID, 484));
    CHECK(image_metadata_valid(E3_BOARD_ID, 488));
    return 0;
}
...
static int test_vectors_and_commit(void) {
    ...
    payload(bytes, E3_SRAM_END, E3_PAYLOAD_ADDRESS + 485);
...
static int test_begin_rejections(void) {
    setup();
    const uint32_t bad_length[] = {0, 484, 489, E3_PAYLOAD_CAPACITY + 4,
                                   UINT32_C(0xFFFFFFFC)};
    for (uint32_t i = 0; i < 5; ++i) {
        clear_output();
    ...
    CHECK(same(output, "ERR IMAGE\n"));
    const char *bad[] = {
        "BEGIN 0103E013 00000010 0000000G\n",
        "BEGIN 0103E013 00000010 0000000\n",
        "BEGIN 0103E013 00000010 000000000\n",
        "BEGIN 0103E013X00000010 00000000\n",
        "BEGIN 0103E013 00000010X00000000\n",
        "BEGIN 0103E013 00000010 -0000000\n",
    };
...
static int test_data_and_end_rejections(void) {
    ...
    setup();
    begin(E3_BOARD_ID, 508, 0);
    all_data(bytes, 448);
    clear_output();
    data(448, bytes + 448, 64);
    CHECK(same(output, "ERR OFFSET\n") && programmed == 112);
...
static int test_virtual_rejections(void) {
    static const char *const malformed[] = {
        "M3 S1\n", "M4 S1\n", "G0 Z1\n", "G1 Z1\n", "G28\n", "G30\n",
        "M280 P0 S10\n", "M106 S255\n", "M104 S200\n", "M140 S60\n",
        "BEGIN 0103E013 00000010 00000000\n", "BOOT\n", "M5 M3 S1\n",
        "M115 X1\n", "UPDATE NOW\n", "INFO\rUPDATE\n", "\n",
        "SIM FAN 1\n", "SIM  FAN 1 1\n", "SIM FAN 1 1 \n", " SIM RESET\n",
