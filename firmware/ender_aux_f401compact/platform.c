...
{
    const uint32_t device = REG32(0xE0042000) & 0xFFFU;
    const uint16_t size = REG16(0x1FFF7A22);
    return (device == 0x423U && size == 256U)
        || (device == 0x433U && size == 512U);
}
...
void platform_report_hardware(void)
{
    ...
    report_text("E3HW:1 MCU:");
    for (int shift = 12; shift >= 0; shift -= 4)
        platform_putchar(digits[(device >> (unsigned)shift) & 15U]);
    report_text(" FLASH_KIB:");
    char decimal[5];
    unsigned count = 0;
    uint32_t value = size;
    do {
    ...
    while (count) platform_putchar(decimal[--count]);
    report_text(" LAYOUT:F401_COMPACT_V1\n");
}
...
{
    /* sector=5 erases only the shared application sector; sector=0 programs an app word. */
...
bool platform_erase_application(void)
{
    ...
    platform_safe_outputs();
    for (unsigned sector = 5; sector <= 5; ++sector) {
        const uint32_t saved = irq_save();
