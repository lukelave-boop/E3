...
class PlatformModel:
    ...
    def __init__(self, elf_path: Path, device=0x423, size=256):
        ...
        self.uc.mem_map(FLASH_START, 0x80000)
        ...
        self.uc.mem_write(FLASH_START, b"\xff" * 0x80000)
        self.default_device, self.default_size = device, size
        ...
        self.uc.hook_add(UC_HOOK_MEM_WRITE, self._program_flash, begin=FLASH_START, end=0x0807FFFF)
        self.set_target()

    def word(self, address: int) -> int:
        return struct.unpack("<I", self.uc.mem_read(address, 4))[0]

    def set_target(self, *, device=None, size=None, ready: bool = True) -> None:
    ...
    def _write_mmio(self, uc, _access, address, size, value, _data):
        if address == FLASH_KEYR:
            ...
            if value & (1 << 16):
                ...
                sector = (value >> 3) & 15
                require(sector == 5, f"Production attempted protected sector erase: {sector}")
    ...
    def call(self, name: str, *args: int) -> int:
        ...
        require(len(args) <= 3, "Harness supports at most three arguments")
        ...
        for register, argument in zip((UC_ARM_REG_R0, UC_ARM_REG_R1, UC_ARM_REG_R2), args, strict=False):
...
def run_tests(model: PlatformModel) -> int:
    ...
    for target in ({"device": 0x423, "size": 512}, {"device": 0x433, "size": 256},
                   {"device": 0x423, "size": 128}, {"device": 0}, {"size": 384}, {"ready": False}):
    ...
    require(model.erased == [5], "Application erase did not touch exactly sector 5")
    ...
    require(bytes(model.uc.mem_read(FLASH_END, 0x40000)) == b"\xff" * 0x40000,
            "Compact updater touched upper RE flash outside its declared layout")
    cases += 1
...
def main() -> int:
    ...
    parser.add_argument("--elf", type=Path, default=Path(__file__).resolve().parents[2] / "build/ender_aux_f401compact/updater.elf")
    args = parser.parse_args()
    require(args.elf.is_file(), f"Build the production updater first: {args.elf}")
    cases = sum(run_tests(PlatformModel(args.elf, device, size))
                for device, size in ((0x423, 256), (0x433, 512)))
