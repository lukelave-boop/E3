...
from firmware.ender_aux.host import FLASH_END, VECTOR_BASE, pack_image, validate_image  # noqa: E402
...
def audit(elf_path: Path, objdump: Path) -> bytes:
    ...
    with elf_path.open("rb") as handle:
        ...
        if vectors is None or vectors["sh_addr"] != VECTOR_BASE or vectors["sh_size"] != 404:
...
def main() -> None:
    ...
    image = audit(args.elf, args.objdump)
    digest = hashlib.sha256(image).hexdigest()
    folder = args.output / f"marlin-material-v1-{digest[:8]}"
    folder.mkdir(parents=True, exist_ok=True)
    (folder / "application.e3fw").write_bytes(image)
    (folder / "firmware.elf").write_bytes(args.elf.read_bytes())
    baseline = json.loads((Path(__file__).parent / "baseline.json").read_text())
    manifest = {
        "baseline": baseline, "sha256": digest,
        "payload_bytes": len(validate_image(image)), "vector_address": hex(VECTOR_BASE),
        "updater": "existing 0.1.0, unchanged", "physical_probing_verified": False,
        "source_revision": subprocess.check_output(["git", "rev-parse", "HEAD"], cwd=ROOT, text=True).strip(),
        "patch_sha256": {str(p.relative_to(Path(__file__).parent)).replace("\\", "/"):
                         hashlib.sha256(p.read_bytes()).hexdigest()
                         for p in sorted(Path(__file__).parent.rglob("*"))
                         if p.suffix in {".patch", ".inc", ".h", ".json"}},
    }
    (folder / "manifest.json").write_text(json.dumps(manifest, indent=2) + "\n")
    print(json.dumps({"package": str(folder.resolve()), **manifest}, indent=2))
