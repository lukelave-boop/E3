...
class LaserFocusPanel(QtWidgets.QWidget):
    ...
    def __init__(self, parent: QtWidgets.QWidget | None = None, *, calibration_mode: bool = False) -> None:
        ...
        self.reference.clicked.connect(lambda: self.request("reference" if self._reference_only() else "home_reference"))
    ...
    def _sync(self) -> None:
        ...
        self.reference.setText("Reference border" if reference_only else "Home / park + reference")
    ...
    def request(self, action: str, value: float | None = None) -> None:
        ...
        button = self.up if action == "jog" and value and value > 0 else self.down if action == "jog" else {
            "reference": self.reference, "home_reference": self.reference, "measure": self.measure, "teach": self.teach,
        ...
        if action in {"reference", "home_reference"} and (action == "reference") != self._reference_only():
            return
        ...
        if action in {"recover", "recover_xy"}:
            self._reset_checks(self.path_clear, self.gauge, self.gauge_removed)
        if action in {"jog", "home_reference", "reference", "measure", "move", "clearance",
...
class LaserFocusCoordinator(QtCore.QObject):
    ...
    def request(self, action: str, arguments: dict[str, Any]) -> None:
        ...
        if action == "recover_xy" and (arguments.get("confirmed") is not True
            ...
            return
        if action in {"reference", "home_reference"} and (
            arguments.get("confirmed") is not True or not self.panel.reference.isEnabled()
            or (action == "reference") != self.panel._reference_only()
        ):
            return
