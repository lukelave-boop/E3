"""Install the exact workpiece-focus companion; default is read-only."""
...
def install(project, *, bundle=None, apply=False):
    bundle = Path(__file__).parent if bundle is None else Path(bundle)
    ...
    candidates = set(range(len(predecessors)))
    ...
    for entry in entries:
        ...
        current = digest == entry["sha256_lf"]
        if not current:
            matching = {index for index, previous in enumerate(predecessors)
                        if relative in previous["files"] and previous["files"][relative] == digest}
            if not matching:
                raise ValueError(f"Preserving unknown local changes: {relative} SHA256={digest}")
            candidates.intersection_update(matching)
            if not candidates:
                raise ValueError("Preserving mixed predecessor revisions; no complete known baseline matches")
        prepared.append((target, original, source, current))
    if apply:
        inactive()
    results = []
    ...
    return {"applied": apply, "files": results, "service_started": False,
            "compatible_predecessors": [predecessors[index]["revision"] for index in sorted(candidates)]}
...
def main():
    ...
    try:
        ...
        print(f"Workpiece-focus support not installed: {exc}", file=sys.stderr)
        return 2
    return 0
