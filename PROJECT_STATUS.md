...
- real laser-disabled motion and low-power framing behavior.
...
Do not enable `machine.allow_motion` until the camera and controller probes have
been captured and reviewed. Normal application launch is already
hardware-capable, but it does not automatically connect. Keep the laser power
...
located zero-power motion program generated by the application, followed by repeatability
